...
infowriter::infowriter(FILE *info, int useplaintext, const char *directory)
	: info(info), directory(directory), useplaintext(useplaintext)
{
...
void infowriter::flush()
{
	...
	for (const auto &box : boxes) {
		...
		case Box::isspriteextension: {
			if (box->type == Box::issprite) {
				fprintf(info, "%5d ", spriteno++);
			} else {
				fprintf(info, "    | ");
			}
			const BoxSprite *s = static_cast<const BoxSprite *>(box.get());
		...
		case Box::isdata:	{
			fprintf(info, "%5d ", spriteno++);
			const BoxData *d = static_cast<const BoxData *>(box.get());
			int instr = 0;
			int count = 0;
...
{
	boxes.push_back(std::make_unique<BoxSprite>(first, filename, x, y, info));
}

void infowriter::adddata(uint size, U8 *data)
{
	boxes.push_back(std::make_unique<BoxData>(size, data));
}
